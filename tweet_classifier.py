...
def clean_data(full_train_data, full_test_data):
    ...
    for i in range(len(full_train_data['text'])):
        ...
        full_train_data['text'].iloc[i] = re.sub('www.|https://|http://|.com|t.co/','',full_train_data['text'].iloc[i])
        # remove all punctuation
        full_train_data['text'].iloc[i] = ''.join([j for j in full_train_data['text'].iloc[i] if j not in string.punctuation])
        # remove all digits
        full_train_data['text'].iloc[i] = ''.join([j for j in full_train_data['text'].iloc[i] if j not in string.digits])
        # remove stopwordstest
        full_train_data['text'].iloc[i] = ' '.join([j for j in full_train_data['text'].iloc[i].split() if j not in stopwords.words('english')])
        # remove non ASCII characters
        full_train_data['text'].iloc[i] = ''.join([j for j in full_train_data['text'].iloc[i] if ord(j) < 128])

    for i in range(len(full_test_data['text'])):
        # remove website links
        full_test_data['text'].iloc[i] = re.sub('www.|https://|http://|.com|t.co/','',full_test_data['text'].iloc[i])
        # remove all punctuation
        full_test_data['text'].iloc[i] = ''.join([j for j in full_test_data['text'].iloc[i] if j not in string.punctuation])
        # remove all digits
        full_test_data['text'].iloc[i] = ''.join([j for j in full_test_data['text'].iloc[i] if j not in string.digits])
        # remove stopwordstest
        full_test_data['text'].iloc[i] = ' '.join([j for j in full_test_data['text'].iloc[i].split() if j not in stopwords.words('english')])
        # remove non ASCII characters
        full_test_data['text'].iloc[i] = ''.join([j for j in full_test_data['text'].iloc[i] if ord(j) < 128])
